			...
			return typeof __filename < "u" && (e = e || __filename), function(t = {}) {
				...
				var Rt = typeof TextDecoder < "u" ? new TextDecoder("utf8") : void 0, cr = (u, m, y) => {
					...
					for (var D = ""; m < R;) {
						...
						var q = u[m++] & 63;
						if ((F & 224) == 192) {
							D += String.fromCharCode((F & 31) << 6 | q);
							continue;
						}
						var J = u[m++] & 63;
						if ((F & 240) == 224 ? F = (F & 15) << 12 | q << 6 | J : F = (F & 7) << 18 | q << 12 | J << 6 | u[m++] & 63, F < 65536) D += String.fromCharCode(F);
						else {
							var _e = F - 65536;
				...
				var jt = 0, Vs = (u, m, y) => {
					...
					normalize: (u) => {
						var m = Le.isAbs(u), y = u.substr(-1) === "/";
						return u = Le.normalizeArray(u.split("/").filter((x) => !!x), !m).join("/"), !u && !m && (u = "."), u && y && (u += "/"), (m ? "/" : "") + u;
					},
					dirname: (u) => {
						var m = Le.splitPath(u), y = m[0], x = m[1];
						return !y && !x ? "." : (x && (x = x.substr(0, x.length - 1)), y + x);
					},
					basename: (u) => {
						if (u === "/") return "/";
						u = Le.normalize(u), u = u.replace(/\/$/, "");
						var m = u.lastIndexOf("/");
						return m === -1 ? u : u.substr(m + 1);
					},
					join: function() {
						var u = Array.prototype.slice.call(arguments);
						return Le.normalize(u.join("/"));
					},
					join2: (u, m) => Le.normalize(u + "/" + m)
				}, ga = () => {
					...
					resolve: function() {
						...
						return u = Le.normalizeArray(u.split("/").filter((R) => !!R), !m).join("/"), (m ? "/" : "") + u || ".";
					},
					relative: (u, m) => {
						...
						for (var x = y(u.split("/")), R = y(m.split("/")), D = Math.min(x.length, R.length), F = D, q = 0; q < D; q++) if (x[q] !== R[q]) {
							F = q;
							break;
						}
						for (var J = [], q = F; q < x.length; q++) J.push("..");
						return J = J.concat(R.slice(F)), J.join("/");
					}
				}, _a = [], ya = (u) => {
					...
					if (!(x > 0)) return 0;
					for (var R = y, D = y + x - 1, F = 0; F < u.length; ++F) {
						var q = u.charCodeAt(F);
						if (q >= 55296 && q <= 57343) {
							var J = u.charCodeAt(++F);
...
`));
					...
					stream_ops: {
						read(u, m, y, x, R) {
							...
							var F = Math.min(u.node.usedBytes - R, x);
							if (F > 8 && D.subarray) m.set(D.subarray(R, R + F), y);
							else for (var q = 0; q < F; q++) m[y + q] = D[R + q];
							return F;
						},
						write(u, m, y, x, R, D) {
							if (!x) return 0;
							var F = u.node;
							if (F.timestamp = Date.now(), m.subarray && (!F.contents || F.contents.subarray)) {
								if (D) return F.contents = m.subarray(y, y + x), F.usedBytes = x, x;
								if (F.usedBytes === 0 && R === 0) return F.contents = m.slice(y, y + x), F.usedBytes = x, x;
								if (R + x <= F.usedBytes) return F.contents.set(m.subarray(y, y + x), R), x;
							}
							if ($e.expandFileStorage(F, R + x), F.contents.subarray && m.subarray) F.contents.set(m.subarray(y, y + x), R);
							else for (var q = 0; q < x; q++) F.contents[R + q] = m[y + q];
							return F.usedBytes = Math.max(F.usedBytes, R + x), x;
						},
						llseek(u, m, y) {
							var x = m;
						...
						mmap(u, m, y, x, R) {
							...
							var D, F, q = u.node.contents;
							if (!(R & 2) && q.buffer === X.buffer) F = !1, D = q.byteOffset;
							else {
								if ((y > 0 || y + m < q.length) && (q.subarray ? q = q.subarray(y, y + m) : q = Array.prototype.slice.call(q, y, y + m)), F = !0, D = ba(), !D) throw new b.ErrnoError(48);
								X.set(q, D);
							}
							return {
								ptr: D,
				...
				}, Sn = (u, m, y, x, R, D, F, q, J, _e) => {
					var Se = m ? Bt.resolve(Le.join2(u, m)) : u;
					function ae(te) {
						function me(Re) {
					...
					Ye(), typeof y == "string" ? $n(y, (te) => ae(te), F) : ae(y);
				}, En = (u) => {
					var m = {
						r: 0,
						"r+": 2,
					...
					lookupPath(u, m = {}) {
						...
						for (var y = u.split("/").filter((_e) => !!_e), x = b.root, R = "/", D = 0; D < y.length; D++) {
							var F = D === y.length - 1;
							if (F && m.parent) break;
							if (x = b.lookupNode(x, y[D]), R = Le.join2(R, y[D]), b.isMountpoint(x) && (!F || F && m.follow_mount) && (x = x.mounted.root), !F || m.follow) for (var q = 0; b.isLink(x.mode);) {
								var J = b.readlink(R);
								if (R = Bt.resolve(Le.dirname(R), J), x = b.lookupPath(R, { recurse_count: m.recurse_count + 1 }).node, q++ > 40) throw new b.ErrnoError(32);
							}
						}
						return {
							path: R,
					...
					syncfs(u, m) {
						...
						function R(F) {
							return b.syncFSRequests--, m(F);
						}
						function D(F) {
							if (F) return D.errored ? void 0 : (D.errored = !0, R(F));
							++x >= y.length && R(null);
						}
						y.forEach((F) => {
							if (!F.type.syncfs) return D(null);
							F.type.syncfs(F, u, D);
						});
					},
					mount(u, m, y) {
						...
						if (!x && !R) {
							var F = b.lookupPath(y, { follow_mount: !1 });
							if (y = F.path, D = F.node, b.isMountpoint(D)) throw new b.ErrnoError(10);
					...
					unmount(u) {
						...
						Object.keys(b.nameTable).forEach((F) => {
							for (var q = b.nameTable[F]; q;) {
								var J = q.name_next;
					...
					mknod(u, m, y) {
						var x = b.lookupPath(u, { parent: !0 }).node, R = Le.basename(u);
					...
					symlink(u, m) {
						...
						var x = Le.basename(m), R = b.mayCreate(y, x);
						if (R) throw new b.ErrnoError(R);
						if (!y.node_ops.symlink) throw new b.ErrnoError(63);
						return y.node_ops.symlink(y, x, u);
					},
					rename(u, m) {
						var y = Le.dirname(u), x = Le.dirname(m), R = Le.basename(u), D = Le.basename(m), F = b.lookupPath(u, { parent: !0 }), q = F.node, J;
						if (F = b.lookupPath(m, { parent: !0 }), J = F.node, !q || !J) throw new b.ErrnoError(44);
					...
					rmdir(u) {
						var m = b.lookupPath(u, { parent: !0 }).node, y = Le.basename(u), x = b.lookupNode(m, y), R = b.mayDelete(m, y, !0);
						if (R) throw new b.ErrnoError(R);
					...
					unlink(u) {
						...
						var y = Le.basename(u), x = b.lookupNode(m, y), R = b.mayDelete(m, y, !1);
						if (R) throw new b.ErrnoError(R);
					...
					open(u, m, y) {
						...
						return F.stream_ops.open && F.stream_ops.open(F), r.logReadFiles && !(m & 1) && (b.readFiles || (b.readFiles = {}), u in b.readFiles || (b.readFiles[u] = 1)), F;
					},
					close(u) {
					...
					read(u, m, y, x, R) {
						...
						var F = u.stream_ops.read(u, m, y, x, R);
						return D || (u.position += F), F;
					},
					write(u, m, y, x, R, D) {
						if (x < 0 || R < 0) throw new b.ErrnoError(28);
						if (b.isClosed(u)) throw new b.ErrnoError(8);
					...
					createSpecialDirectories() {
						...
						b.mkdir("/proc/self/fd"), b.mount({ mount() {
							...
							return m.node_ops = { lookup(y, x) {
								var R = +x, D = b.getStreamChecked(R), F = {
									parent: null,
					...
					analyzePath(u, m) {
						...
						try {
							...
							x.parentExists = !0, x.parentPath = y.path, x.parentObject = y.node, x.name = Le.basename(u), y = b.lookupPath(u, { follow: !m }), x.exists = !0, x.path = y.path, x.object = y.node, x.name = y.node.name, x.isRoot = y.path === "/";
						} catch (R) {
							x.error = R.errno;
						}
						return x;
					...
					createFile(u, m, y, x, R) {
						var D = Le.join2(typeof u == "string" ? u : b.getPath(u), m), F = Hr(x, R);
						return b.create(D, F);
					},
					createDataFile(u, m, y, x, R, D) {
						var F = m;
						u && (u = typeof u == "string" ? u : b.getPath(u), F = m ? Le.join2(u, m) : u);
						var q = Hr(x, R), J = b.create(F, q);
						if (y) {
							if (typeof y == "string") {
					...
					createDevice(u, m, y, x) {
						var R = Le.join2(typeof u == "string" ? u : b.getPath(u), m), D = Hr(!!y, !!x);
						b.createDevice.major || (b.createDevice.major = 64);
						var F = b.makedev(b.createDevice.major++, 0);
						return b.registerDevice(F, {
							open(q) {
								q.seekable = !1;
							},
							close(q) {
					...
					doStat(u, m, y) {
						try {
							...
							if (q && q.node && Le.normalize(m) !== Le.normalize(b.getPath(q.node))) return -54;
							throw q;
						}
						...
						var R = x.atime.getTime(), D = x.mtime.getTime(), F = x.ctime.getTime();
						return je = [Math.floor(R / 1e3) >>> 0, (ye = Math.floor(R / 1e3), +Math.abs(ye) >= 1 ? ye > 0 ? +Math.floor(ye / 4294967296) >>> 0 : ~~+Math.ceil((ye - +(~~ye >>> 0)) / 4294967296) >>> 0 : 0)], K[y + 40 >> 2] = je[0], K[y + 44 >> 2] = je[1], U[y + 48 >> 2] = R % 1e3 * 1e3, je = [Math.floor(D / 1e3) >>> 0, (ye = Math.floor(D / 1e3), +Math.abs(ye) >= 1 ? ye > 0 ? +Math.floor(ye / 4294967296) >>> 0 : ~~+Math.ceil((ye - +(~~ye >>> 0)) / 4294967296) >>> 0 : 0)], K[y + 56 >> 2] = je[0], K[y + 60 >> 2] = je[1], U[y + 64 >> 2] = D % 1e3 * 1e3, je = [Math.floor(F / 1e3) >>> 0, (ye = Math.floor(F / 1e3), +Math.abs(ye) >= 1 ? ye > 0 ? +Math.floor(ye / 4294967296) >>> 0 : ~~+Math.ceil((ye - +(~~ye >>> 0)) / 4294967296) >>> 0 : 0)], K[y + 72 >> 2] = je[0], K[y + 76 >> 2] = je[1], U[y + 80 >> 2] = F % 1e3 * 1e3, je = [x.ino >>> 0, (ye = x.ino, +Math.abs(ye) >= 1 ? ye > 0 ? +Math.floor(ye / 4294967296) >>> 0 : ~~+Math.ceil((ye - +(~~ye >>> 0)) / 4294967296) >>> 0 : 0)], K[y + 88 >> 2] = je[0], K[y + 92 >> 2] = je[1], 0;
					},
					doMsync(u, m, y, x, R) {
				...
				function Tn(u, m, y) {
					...
					try {
						...
						if (typeof b > "u" || F.name !== "ErrnoError") throw F;
						return -F.errno;
					}
				}
				...
				function In(u, m, y) {
					try {
						...
						for (var R = 280, D = 0, F = b.llseek(x, 0, 1), q = Math.floor(F / R); q < x.getdents.length && D + R <= y;) {
						...
						switch (m) {
							...
							case 21508:
								...
								if (x.tty.ops.ioctl_tcsets) {
									for (var ae = Ue.getp(), F = K[ae >> 2], q = K[ae + 4 >> 2], J = K[ae + 8 >> 2], _e = K[ae + 12 >> 2], Se = [], D = 0; D < 32; D++) Se.push(X[ae + D + 17 >> 0]);
				...
				var Ys = (u, m, y, x) => {
					for (var R = 0, D = 0; D < y; D++) {
						var F = U[m >> 2], q = U[m + 4 >> 2];
						m += 8;
						var J = b.read(u, X, F, q, x);
						if (J < 0) return -1;
						if (R += J, J < q) break;
					}
					return R;
				...
				function eo(u, m, y, x, R) {
					...
					try {
						...
						var F = Ue.getStreamFromFD(u);
						return b.llseek(F, D, x), je = [F.position >>> 0, (ye = F.position, +Math.abs(ye) >= 1 ? ye > 0 ? +Math.floor(ye / 4294967296) >>> 0 : ~~+Math.ceil((ye - +(~~ye >>> 0)) / 4294967296) >>> 0 : 0)], K[R >> 2] = je[0], K[R + 4 >> 2] = je[1], F.getdents && D === 0 && x === 0 && (F.getdents = null), 0;
					} catch (q) {
				...
				var Xt = (u) => u % 4 === 0 && (u % 100 !== 0 || u % 400 === 0), ro = (u, m) => {
					...
					for (var J in q) F = F.replace(new RegExp(J, "g"), q[J]);
					var _e = [
						"Sunday",
						"Monday",
						"Tuesday",
...
`,
					...
					F = F.replace(/%%/g, "\0\0");
					for (var J in Te) F.includes(J) && (F = F.replace(new RegExp(J, "g"), Te[J](D)));
					F = F.replace(/\0\0/g, "%");
					var Be = Vr(F, !1);
				...
				var Br = !1;
				return r.noInitialRun && (Br = !1), Fn(), t.ready;
			};
		})();
	})), y0 = ko({
		InferenceSession: () => Jn,
...
`));
		...
		async function ot() {
			function o(g, S) {
				...
				})(), bn.push(Yt._b), gg = (g = Yt).tb, _g = g.ub, t._OrtInit = g.vb, t._OrtGetLastError = g.wb, t._OrtCreateSessionOptions = g.xb, t._OrtAppendExecutionProvider = g.yb, t._OrtAddFreeDimensionOverride = g.zb, t._OrtAddSessionConfigEntry = g.Ab, t._OrtReleaseSessionOptions = g.Bb, t._OrtCreateSession = g.Cb, t._OrtReleaseSession = g.Db, t._OrtGetInputOutputCount = g.Eb, t._OrtGetInputOutputMetadata = g.Fb, t._OrtFree = g.Gb, t._OrtCreateTensor = g.Hb, t._OrtGetTensorData = g.Ib, t._OrtReleaseTensor = g.Jb, t._OrtCreateRunOptions = g.Kb, t._OrtAddRunConfigEntry = g.Lb, t._OrtReleaseRunOptions = g.Mb, t._OrtCreateBinding = g.Nb, t._OrtBindInput = g.Ob, t._OrtBindOutput = g.Pb, t._OrtClearBoundOutputs = g.Qb, t._OrtReleaseBinding = g.Rb, t._OrtRunWithBinding = g.Sb, t._OrtRun = g.Tb, t._OrtEndProfiling = g.Ub, t._JsepOutput = g.Vb, t._JsepGetNodeName = g.Wb, Wn = g.Xb, Lt = t._free = g.Yb, Ia = t._malloc = g.Zb, fo = g.ac, yg = g.bc, wg = g.cc, bg = g.dc, ho = g.ec, vg = g.fc, $g = g.gc, ze = g.hc, za = g.ic, xg = g.jc, Ee = g.kc, go = g.lc, Ie = g.mc, kg = g.nc, _o = g.oc, Sg = g.pc, Eg = g.qc, Tg = g.rc, yo = g.sc, Ig = g.tc, zg = g.uc, Cg = g.vc, Ag = g.wc, Rg = g.xc, Og = g.yc, Bg = g.zc, Dg = g.Ac, Mg = g.Bc, Ng = g.Cc, Pg = g.Dc, Ug = g.Ec, Fg = g.Fc, Lg = g.Gc, qg = g.Hc, Wg = g.Ic, Gg = g.Jc, Vg = g.Kc, Hg = g.Lc, jg = g.Mc, Kg = g.Nc, Xg = g.Pc, Zg = g.Qc, Yg = g.$c, Qg = g.ad, Jg = g.fd, e0 = g.jd, t0 = g.kd, r0 = g.ld, a0 = g.md, n0 = g.nd, i0 = g.od, s0 = g.pd, o0 = g.qd, u0 = g.vd, l0 = g.Td, d0 = g.Ud, p0 = g.Vd, c0 = g.Wd, v = S, Yt;
			}
			var p, _ = lt();
		...
		function Gs(o) {
			var p = new ga(o >>>= 0);
			return (T(), X)[p.Tc + 12 >>> 0] == 0 && (vn(p, !0), Wr--), Le(p, !1), Ot.push(p), Eg(o);
		}
		var jt = 0, Vs = () => {
			ze(0, 0);
			var o = Ot.pop();
		...
		var wa = () => {
			...
			var p = o.cd;
			throw (T(), X)[o.Tc + 13 >>> 0] == 0 && (Ot.push(o), Le(o, !0), vn(o, !1), Wr++), _o(p), jt = p;
		};
		function Vr(o, p, _) {
			var g = new ga(o >>>= 0);
		...
		function ro(o, p) {
			Dt(o >>>= 0, {
				...
				Vc(_, g) {
					...
					return (T(), j)[M >>> 2 >>> 0] = O, S ? Ft(g, W, O + 1) : (T(), ue).set(g, W >>> 0), _ !== null && _.push(Lt, M), M;
				},
				Uc: Sa,
				Wc(_) {
					Lt(_);
		...
		var m = 0, y = null, x = 0, R = [], D = /* @__PURE__ */ new Map(), F = /* @__PURE__ */ new Map(), q = /* @__PURE__ */ new Map(), J = 0, _e = null, Se = [], ae = (o) => (function(p) {
			if (!N) {
				if (m === 0) {
					var _ = !1, g = !1;
					p((S = 0) => {
						if (!N && (x = S, _ = !0, g)) {
							...
							try {
								var O = (function() {
									var Q = (T(), U)[y + 8 >>> 2 >>> 0];
									return Q = F.get(Q), Q = q.get(Q), --bt, Q();
								})();
							} catch (Q) {
								O = Q, S = !0;
							}
				...
				} else m === 2 ? (m = 0, Br(c0), Lt(y), y = null, Se.forEach(Zt)) : ee(`invalid state: ${m}`);
				return x;
			}
		})((p) => {
			o().then(p);
		...
		var me = [], Re = (o) => {
			...
			for (var _ = Array(o), g = 0; g < o; ++g) {
				...
				if (M === void 0) throw o = `parameter ${g}`, O = gg(O), p = vt(O), Lt(O), new mr(`${o} has unknown type ${p}`);
				_[S] = M;
			}
			return _;
		}, Te = (o, p, _) => {
  ...
  ${W}
		...
		var Ta = {}, Y0 = () => performance.timeOrigin + performance.now();
		function sg(o, p) {
		...
		function Q0(o, p, _, g) {
			...
			})(O), p = p(S), S < O ? (Ft(o, _, 17), Ft(p, g, 17)) : (Ft(o, g, 17), Ft(p, _, 17));
		}
		var J0 = () => Date.now(), e_ = 1;
		function t_(o, p, _) {
		...
		var u_ = () => navigator.hardwareConcurrency, Dr = {}, Ln = (o) => {
			var p;
			return (p = /\bwasm-function\[\d+\]:(0x[0-9a-f]+)/.exec(o)) ? +p[1] : (p = /:(\d+):\d+(?:\)|$)/.exec(o)) ? 2147483648 | +p[1] : 0;
		}, ug = (o) => {
			for (var p of o) (o = Ln(p)) && (Dr[o] = p);
		};
		function l_() {
			var o = Error().stack.toString().split(`
`);
			return o[0] == "Error" && o.shift(), ug(o), Dr.gd = Ln(o[3]), Dr.Jd = o, Dr.gd;
		}
		function qn(o) {
			...
			Lt(qn.hd ?? 0), p = Xt(o) + 1;
			var _ = Ia(p);
			return _ && Ft(o, _, p), qn.hd = _, qn.hd;
		}
		function d_(o) {
			o >>>= 0;
			var p = (T(), ue).length;
...
`))[0] == "Error" && g.shift(), ug(g);
			for (var S = 3; g[S] && Ln(g[S]) != o;) ++S;
			for (o = 0; o < _ && g[o + S]; ++o) (T(), U)[p + 4 * o >>> 2 >>> 0] = Ln(g[o + S]);
			return o;
		}
		var co, mo = {}, lg = () => {
			if (!co) {
		...
		function dg(o, p) {
			...
			for (_ of lg()) {
				var O = p + g;
				(T(), j)[o + S >>> 2 >>> 0] = O, g += Ft(_, O, 1 / 0) + 1, S += 4;
			}
			return 0;
		}
		function pg(o, p) {
		...
		function hg(o, p, _, g) {
			...
			for (var S = 0, O = 0; O < _; O++) {
				...
				p += 8;
				for (var Q = 0; Q < W; Q++) {
					var re = o, xe = (T(), ue)[M + Q >>> 0], De = c_[re];
		...
		}, t.UTF8ToString = rt, t.stringToUTF8 = Ft, t.lengthBytesUTF8 = Xt;
		var gg, _g, Wn, Lt, Ia, fo, yg, wg, bg, ho, vg, $g, ze, za, xg, Ee, go, Ie, kg, _o, Sg, Eg, Tg, yo, Ig, zg, Cg, Ag, Rg, Og, Bg, Dg, Mg, Ng, Pg, Ug, Fg, Lg, qg, Wg, Gg, Vg, Hg, jg, Kg, Xg, Zg, Yg, Qg, Jg, e0, t0, r0, a0, n0, i0, s0, o0, u0, l0, d0, p0, c0, Yt, f_ = [
			lr,
			dr,
			Ut,
			Sn,
		...
		function O_(o, p, _, g, S, O, M, W, Q, re, xe, De) {
			var qe = Ie();
			try {
				Fg(o, p, _, g, S, O, M, W, Q, re, xe, De);
			} catch (Ke) {
				if (Ee(qe), Ke !== Ke + 0) throw Ke;
				ze(1, 0);
			}
		}
		function B_(o, p, _, g, S, O) {
			var M = Ie();
			try {
				return Lg(o, p, _, g, S, O);
			} catch (W) {
				if (Ee(M), W !== W + 0) throw W;
				ze(1, 0);
			}
		...
		function K_(o, p, _, g, S, O, M, W, Q, re, xe, De) {
			var qe = Ie();
			try {
		...
		function X_(o, p, _, g, S, O, M, W, Q, re, xe) {
			var De = Ie();
			try {
		...
		function Z_(o, p, _, g, S, O, M, W, Q, re, xe, De, qe, Ke, fr, bo) {
			var e1 = Ie();
			try {
				Jg(o, p, _, g, S, O, M, W, Q, re, xe, De, qe, Ke, fr, bo);
			} catch (vo) {
				if (Ee(e1), vo !== vo + 0) throw vo;
				ze(1, 0);
			}
		}
		function Y_(o, p, _) {
	...
	var Ra, To, Io, zo, Co, Z, hr, Ao, Mr, Yr, Jt, gr, Ro, Oo, Hn, Bo, Do, Mo, jn, it, Kn, He, No, Po, Uo, Fo, Oa, Lo, qo, Wo, Go, Vo, Ho, _r, Qr, Xn, jo, Ko, Xo, Zo, Yo, ft, Ba, xt, Zn, Jr, Yn, kt, yt, er, tr, Qn, Qo, Jo, Jn, eu, tu, ru, au, nu, ei, ht, Da, ti, ri, ai, iu, su, ni, ou, uu, lu, ii, si, du, gt, pu, Ma, cu, mu, oi, fu, ui, hu, li, gu, Na, di, Pa, ea, pi, _u, yu, wu, ci, We, rr, St, Ua, Fe, Fa, bu, vu, $u, xu, ku, yr, Su, Eu, Tu, wr, qt, br, La, qa, mi, fi, hi, fe, gi, _i, Iu, zu, Cu, Au, yi, Ru, Ce, Mt, Ou, Nr, L, Wa, Bu, Du, Mu, we, wi, bi, vi, $i, xi, Nu, ki, Pu, Si, Ei, Ti, Uu, Fu, Lu, ta, qu, Wu, Gu, Ga, Ii, Va, Ha, Vu, Hu, zi, Ci, ju, Ku, Xu, Zu, Ne, Qe, Pr, ja, nt, ut, de, Je, Ai, Ur, ar, le, ra, G, oe, Yu, Ri, Qu, Ju, be, el, Oi, tl, rl, al, nl, _t, il, sl, Wt, ol, ul, ll, dl, pl, cl, ml, fl, hl, gl, Et, _l, yl, wl, bl, vl, $l, xl, kl, Sl, El, Tl, Tt, Il, Ka, Bi, It, zl, Cl, Al, Rl, Ol, Bl, Dl, Ml, Nl, Pl, zt, Ul, Fl, Ll, ql, Wl, Gl, Vl, Hl, jl, Kl, Xa, Di, Xl, Zl, Mi, Yl, Ql, Za, Jl, ed, td, aa, rd, ad, Ya, nd, id, sd, od, ud, ld, dd, pd, cd, md, Me, fd, hd, gd, _d, yd, wd, bd, vd, $d, xd, kd, Sd, Ed, Td, na, Id, Qa, zd, Cd, Ad, Rd, Od, Bd, Dd, Md, Nd, Pd, Ud, Fd, Ld, qd, Wd, Gd, Ni, Vd, Pi, Ui, Hd, jd, Kd, Xd, Zd, Yd, Ja, Qd, Jd, ep, tp, rp, ap, Ct, np, ip, sp, op, up, lp, dp, pp, cp, mp, fp, hp, gp, _p, yp, wp, bp, vp, vr, $r, xr, Fi, nr, st, $p, en, xp, kp, ia, Li, tn, Sp, Ep, qi, Wi, Tp, Gi, Ip, rn, an, zp, Cp, Ap, Rp, Vi, sa, Op, Hi, Bp, Dp, Mp, Np, Pp, Up, Fp, Lp, nn, qp, sn, ji, Ki, Wp, Gp, Xi, Vp, Hp, jp, Kp, Xp, Zp, Zi, Yp, Qp, Yi, Jp, ec, tc, rc, ac, nc, ic, sc, oc, uc, lc, dc, pc, on, oa, Qi, cc, mc, fc, hc, Ji, gc, _c, yc, wc, bc, es, vc, $c, xc, kc, Sc, Ec, Tc, Ic, zc, Cc, Ac, Rc, Oc, Bc, Dc, Mc, Nc, Pc, Uc, Fc, Lc, qc, Wc, Gc, Vc, Hc, jc, Kc, Xc, Zc, Yc, Nt, Gt, kr, Sr, Qc, Jc, em, tm, rm, am, nm, im, sm, om, um, dt, lm, dm, ts, pm, ua, cm, rs, mm, fm, hm, gm, as, _m, ym, ns, wm, un, bm, is, vm, $m, ss, xm, km, Sm, os, Em, Tm, Im, zm, Cm, Am, Rm, Om, Bm, Dm, Mm, Nm, Pm, Um, Fm, Lm, qm, Wm, Gm, Vm, Hm, jm, Km, Xm, Zm, Ym, Qm, la, us, ls, ds, ps, Jm, ef, cs, ms, tf, rf, fs, af, nf, hs, sf, of, uf, lf, df, pf, cf, mf, ff, hf, gf, _f, yf, wf, bf, vf, $f, xf, kf, Sf, Ef, Tf, gs, If, zf, Cf, Af, Rf, Of, Bf, Df, _s, Mf, Nf, Pf, Uf, Ff, Lf, qf, Wf, Gf, Vf, Hf, jf, Kf, da, Xf, ys, Zf, Yf, Qf, Jf, eh, th, rh, ah, nh, ih, ws, sh, oh, uh, lh, dh, ph, ch, mh, fh, hh, gh, _h, yh, bs, wh, bh, vh, $h, xh, vs, ln, kh, Sh, Eh, Th, $s, xs, ir, Ih, ks, dn, Ss, Es, Ts, Is, zs, Cs, As, sr, wt, Fr, pa, ca, pn, Rs, cn, Er, Tr, zh, Ch, Ah, Rh, Oh, Bh, Dh, Mh, Os, Bs, Nh, Ph, Uh, Ds, Ms, Ns, Fh, Lh, qh, Wh, w0 = xo((() => {
		Ra = Object.defineProperty, To = Object.getOwnPropertyDescriptor, Io = Object.getOwnPropertyNames, zo = Object.prototype.hasOwnProperty, Co = ((e) => typeof require < "u" ? require : typeof Proxy < "u" ? new Proxy(e, { get: (t, r) => (typeof require < "u" ? require : t)[r] }) : e)(function(e) {
			...
			rr(), Fa(), bu = (e) => {
				...
				try {
					...
					let i = 0;
					return e?.tag !== void 0 && (i = St(e.tag, a)), r = t._OrtCreateRunOptions(n.logSeverityLevel, n.logVerbosityLevel, !!n.terminate, i), r === 0 && Fe("Can't create run options."), e?.extra !== void 0 && Ua(e.extra, "", /* @__PURE__ */ new WeakSet(), (s, l) => {
						let d = St(s, a), c = St(l, a);
						t._OrtAddRunConfigEntry(r, d, c) !== 0 && Fe(`Can't set a run config entry: ${s} - ${l}.`);
					}), [r, a];
				} catch (i) {
			...
			rr(), Fa(), $u = (e) => {
				...
				We()._OrtAddSessionConfigEntry(e, n, i) !== 0 && Fe(`Can't set a session config entry: ${t} - ${r}.`);
			}, Su = async (e, t, r) => {
				...
				for (let n of a) {
					...
					await We()._OrtAppendExecutionProvider(e, l, c, h, d) !== 0 && Fe(`Can't append execution provider: ${i}.`);
				}
			}, Eu = async (e) => {
				let t = We(), r = 0, a = [], n = e || {};
				ku(n);
				try {
					...
					if (r = t._OrtCreateSessionOptions(i, !!n.enableCpuMemArena, !!n.enableMemPattern, s, !!n.enableProfiling, 0, l, d, c, h), r === 0 && Fe("Can't create session options."), n.executionProviders && await Su(r, n, a), n.enableGraphCapture !== void 0) {
					...
					if (n.freeDimensionOverrides) for (let [f, w] of Object.entries(n.freeDimensionOverrides)) {
						...
						let v = St(f, a);
						t._OrtAddFreeDimensionOverride(r, v, w) !== 0 && Fe(`Can't set a free dimension override: ${f} - ${w}.`);
					}
					return n.extra !== void 0 && Ua(n.extra, "", /* @__PURE__ */ new WeakSet(), (f, w) => {
						yr(r, f, w, a);
					}), [r, a];
				} catch (i) {
					throw r !== 0 && t._OrtReleaseSessionOptions(r) !== 0 && Fe("Can't release session options."), a.forEach((s) => t._free(s)), i;
				}
			};
		}), fe = Z(() => {
			"use strict";
			...
			fe(), Mt(), vi = /* @__PURE__ */ new Map([
				...
				let n = e.byteLength / a, i = new (La(t))(e.buffer, e.byteOffset, n);
				switch (t) {
					case "int64":
					case "uint64": {
			...
			"use strict";
			fe(), rr(), bi(), Lu(), Mt(), ta = /* @__PURE__ */ new Map([
				[1, "float32"],
				[10, "float16"],
				[6, "int32"],
				[12, "uint32"],
  ...
  fn set_${e}(${V}, value: ${h}) {
    set_${e}ByIndices(${P(se)}, value);
				...
				return {
					impl: () => {
						let V = [], se = !1;
						return v.offsetToIndices && (V.push(E), se = !0), v.indicesToOffset && (V.push(B), se = !0), v.broadcastedIndicesToOffset && (Object.values(X).forEach((ke) => V.push(ke)), se = !0), v.set && (V.push(ge), se = !0), v.setByIndices && (V.push(he), se = !0), v.get && (V.push(j), se = !0), v.getByIndices && (V.push(U), se = !0), !i && se && V.unshift(`const ${k} = ${f.indices}(${r.join(",")});`, `const ${C} = ${f.indices}(${L.computeStrides(r).join(",")});`), V.join(`
`);
					},
					type: f,
					offsetToIndices: A,
...
`);
			...
			}, Oi = (e, t) => t.length !== 0 ? t : [...new Array(e).keys()].reverse(), tl = (e, t) => L.sortBasedOnPerm(e, Oi(e.length, t)), rl = (e, t, r, a) => {
  ...
  ${$.mainStart()}
    ...
    output[global_idx] = input[global_idx];
				...
				let { newShape: h, newPerm: f } = al(e.dims, n), w = L.areEqual(f, [
					2,
					3,
					1
				]), v = L.areEqual(f, [
  ...
  ${$.mainStart()}
    ...
    ${C.setByOffset("global_idx", k.getByIndices("aIndices"))}
			...
			fe(), we(), be(), Xa(), Wt(), ol = {
				...
				let l = r[0].dims, d = L.size(i), c = L.size(s), h = G("_A", r[0].dataType, l), f = oe("output", n, i), w = 64;
				d === 1 && (w = 256);
				let v = `
        ...
        fn DIV_CEIL(a : u32, b : u32) -> u32 {
         ...
         if (local_idx == 0u) {
          ${f.setByOffset("outputIndex", `${a === "mean" ? `${f.type.storage}(bestValue / f32(uniforms.reduceSize))` : `${f.type.storage}(${dl[a]})`}`)};
				...
				let s = L.normalizeAxes(i, e.inputs[0].dims.length), l = s, d = e.inputs[0], c = hl(l, e.inputs[0].dims.length);
			...
			fe(), we(), Qe(), be(), Tl(), Tt = (e) => {
				...
				let d = [], c = r[0].dims, h = c.length, f = L.normalizeAxes(n, h), w = !l && f.length === 0;
				c.forEach((k, C) => {
					w || f.indexOf(C) >= 0 ? s && d.push(1) : d.push(k);
				});
				let v = d.length, $ = L.size(d);
				return {
					name: e,
					shaderCache: t,
					getShaderSource: (k) => {
...
var best_index : i32 = 0;`,
      ...
      ${s ? `
        for (var total_seq_id: u32 = seq_causal_length; total_seq_id + local_offset < uniforms.total_sequence_length; total_seq_id++) {
          x[offset + total_seq_id] = ${E.type.value}(${N}(0));
				...
				], B = f && a && L.size(a.dims) > 0, N = ["type", "type"];
    ...
    if (global_id.y < uniforms.M && global_id.x < total_sequence_length) {
      ...
      var sum: f32 = ${(() => {
        ...
        output[outputIdx] = ${U.type.value} (sum * uniforms.alpha) + ${n ? "attention_bias[outputIdx]" : "0.0"};
				...
				], E = f && a && L.size(a.dims) > 0, A = ["type", "type"];
  ...
  ${N.mainStart([
   ...
   if (m < uniforms.M && n < uniforms.N) {
     ...
     output[outputIdx] = value;
				...
				let w = Math.min(e.outputCount, 1 + (s ? 1 : 0) + (l ? 1 : 0)), v = w > 1 ? s : void 0, $ = w > 1 ? l : void 0, k = w > 1 ? c.pastSequenceLength : 0, C = k + c.kvSequenceLength, I = d && L.size(d.dims) > 0 ? d : void 0, E = [t, r];
				v && L.size(v.dims) > 0 && E.push(v), I && E.push(I), h && E.push(h), f && E.push(f);
				...
				let z = [A, a];
				$ && L.size($.dims) > 0 && z.push($), h && z.push(h), f && z.push(f), e.compute(td(w, A, a, $, c, k, h, f), {
					inputs: z,
					outputs: w > 1 ? [0, 2] : [0]
				});
			}, rd = (e, t) => {
    ...
    if (m < uniforms.M && n < uniforms.N) {
      ...
      output_v[outputIdx] = valueV;
			...
			ht(), fe(), we(), Qe(), be(), nd = (e, t) => {
				...
				let { epsilon: r, spatial: a, format: n } = t, i = e[0].dims, s = a ? Je(i[i.length - 1]) : 1, l = n === "NHWC" && i.length > 1 ? s : 1, d = L.size(i) / s, c = a, h = c ? i.length : i, f = G("x", e[0].dataType, e[0].dims, s), w = G("scale", e[1].dataType, e[1].dims, l), v = G("bias", e[2].dataType, e[2].dims, l), $ = G("inputMean", e[3].dataType, e[3].dims, l), k = G("inputVar", e[4].dataType, e[4].dims, l), C = oe("y", e[0].dataType, h, s), I = () => {
					let A = "";
  ...
  ${A.guardAgainstOutOfBoundsWorkgroupSizes("uniforms.outputSize")}
    ...
    ${C.setByOffset("global_idx", "value")}
			...
			we(), be(), ld = (e) => {
				...
				let t = e[0].dims, r = e[0].dims[2], a = L.size(t) / 4, n = e[0].dataType, i = G("input", n, t, 4), s = G("bias", n, [r], 4), l = G("residual", n, t, 4), d = oe("output", n, t, 4);
				return {
					name: "BiasAdd",
					getRunData: () => ({
						outputs: [{
  ...
  ${e.mainStart()}
    ...
    ${h.setByOffset("global_idx", d)}
				...
				let d = [{
					type: 12,
					data: Math.ceil(L.size(e.dims) / 4)
				}];
				return s && d.push(...s), {
					name: t,
					shaderCache: {
					...
					getShaderSource: (c) => md(c, L.size(e.dims), e.dataType, i, r, a, l),
					getRunData: (c) => ({
						outputs: [{
							dims: e.dims,
							dataType: i
						}],
						dispatchGroup: { x: Math.ceil(L.size(c[0].dims) / 64 / 4) },
						programUniforms: d
					})
				};
			}, fd = (e) => {
...
`, Zd = (e) => `quick_gelu_impl(${e})`, Yd = (e, t) => {
			...
			we(), be(), Ja(), Qd = (e) => {
				...
				t[2] = t[2] / 2;
				let r = G("input", e[0].dataType, e[0].dims, 4), a = G("bias", e[0].dataType, [e[0].dims[2]], 4), n = oe("output", e[0].dataType, t, 4), i = L.size(t) / 4, s = nt(e[0].dataType);
				return {
					name: "BiasSplitGelu",
					getRunData: () => ({
						outputs: [{
  ...
  ${l.mainStart()}
    ...
    ${n.setByOffset("global_idx", "valueLeft * geluRight")}
			...
			fe(), we(), be(), rp = (e, t, r, a, n, i, s, l, d, c, h, f) => {
				...
				if (n) if (i) {
					let E = L.size(t) === 1, A = L.size(r) === 1, z = t.length > 0 && t[t.length - 1] % 4 === 0, B = r.length > 0 && r[r.length - 1] % 4 === 0;
        ...
        ${I}
				...
				let l = r.dims.map(Number), d = a.dims.map(Number), c = !L.areEqual(l, d), h = l, f = L.size(l), w = !1, v = !1, $ = [c];
				if (c) {
					let k = Nr.calcShape(l, d, !1);
					if (!k) throw new Error("Can't perform binary op on the given tensors");
					h = k.slice(), f = L.size(h);
					let C = L.size(l) === 1, I = L.size(d) === 1, E = l.length > 0 && l[l.length - 1] % 4 === 0, A = d.length > 0 && d[d.length - 1] % 4 === 0;
...
`);
				...
				let n = L.size(r), i = new Array(e.length), s = new Array(e.length), l = 0, d = [], c = [], h = [{
					type: 12,
					data: n
				}];
  ...
  ${k.mainStart()}
    ...
    ${_p(s, f)}
				...
				let r = e.inputs, a = r[0].dims, n = L.normalizeAxis(t.axis, a.length);
				hp(r, n);
				let i = a.slice();
				i[n] = r.reduce((l, d) => l + (d.dims.length > n ? d.dims[n] : 0), 0);
				let s = r.filter((l) => L.size(l.dims) > 0);
...
`;
				...
				let s = e[0].dims, l = e[1].dims, d = s[s.length - 2], c = l[l.length - 1], h = s[s.length - 1], f = Je(c), w = Je(h), v = Je(d), $ = L.size(r) / f / v, k = e.length > 2, C = a ? a.slice(0, -2) : r.slice(0, -2), I = [
					L.size(C),
					d,
					c
				], E = [
...
`;
    ...
    `;
				...
				let s = e[0].dims, l = e[1].dims, d = s.slice(0, -2), c = l.slice(0, -2), h = a ? a.slice(0, -2) : r.slice(0, -2), f = L.size(h), w = s[s.length - 2], v = s[s.length - 1], $ = l[l.length - 1], k = v % 4 === 0 && $ % 4 === 0, C = w <= 8 ? [
					4,
					1,
					1
				] : [
        ...
        fn getBiasByOutputCoords(coords : array<u32, 5>) -> ${E} {
          ...
          ${k.guardAgainstOutOfBoundsWorkgroupSizes("uniforms.output_size")}
			...
			fe(), we(), be(), nr(), Pp = (e, t, r, a) => {
				let n = e.length > 2, i = n ? "value += b[output_channel];" : "", s = e[0].dims, l = e[1].dims, d = t.format === "NHWC", c = d ? r[3] : r[1], h = c / t.group, f = d && h >= 4 ? Je(c) : 1, w = L.size(r) / f, v = [
					{
						type: 12,
						data: w
					},
  ...
  ${C.mainStart()}
    ...
    ${I.setByOffset("global_idx", "value")}
				...
				let n = e.length > 2, i = Je(r[3]), s = Je(r[2]), l = L.size(r) / i / s, d = [
					e[0].dims[0],
					e[0].dims[1],
					e[0].dims[2],
					e[0].dims[3] / i
  ...
  ${$.mainStart()}
    ...
    for (var i = 0u; i < ${s}u; i++) {
      ...
      ${k.set("batch", "row", "col + i", "output_channel", "value")};
			...
			"use strict";
			we(), Ap(), Np(), an(), Fp(), nr(), tn(), Wt(), Lp = (e, t, r, a, n, i) => {
				...
				let n = r.format === "NHWC", i = Lp(t[0].dims, t[1].dims, r.dilations, r.pads, r.strides, n);
				if (r.group !== 1) {
					let N = [t[0]];
					if (n) {
				...
				if (k || h === 1 && f === 1 && r.dilations[0] === 1 && r.dilations[1] === 1 && r.strides[0] === 1 && r.strides[1] === 1 && r.pads[0] === 0 && r.pads[1] === 0) {
					...
					ue < 8 && ie < 8 ? e.compute(Li(X, r, i, Y, n, a), { inputs: X }) : e.compute(rn(X, r, i, Y, n, a), { inputs: X });
					return;
				}
			...
			fe(), Mt(), we(), be(), Hp = (e, t, r) => {
				let a = e.length > 2, n = t.outputShape, i = t.format === "NHWC", s = t.group, l = e[1].dims, d = l[2] / s, c = l[3], h = i ? Je(d) : 1, f = i && c === 1 && d >= 4, w = f ? Math.floor(d / 4) * 4 : Math.floor(d / h) * h, v = d - w, $ = i ? Je(c) : 1, k = i ? c === 1 ? h : $ : 1, C = L.size(n) / $, I = [
					Math.ceil(C / 64),
					1,
					1
				];
    ...
    ${ke}}`;
			...
			fe(), we(), Qe(), be(), rc = (e, t, r, a) => {
				let n = L.size(t), i = t.length, s = G("input", e, i), l = oe("output", e, i), d = r.dataType === 6 ? r.getInt32Array()[0] : Number(r.getBigInt64Array()[0]), c = L.normalizeAxis(d, i), h = (f) => {
  ...
  ${I.mainStart()}
    ...
    ${k.setByOffset("global_idx", $.getByIndices("aIndices"))}
				...
				return {
					...
					getRunData: (I) => {
						let E = d ? [
							...
							a * c,
							n * c
						], A = L.size(E), z = f.dims, B = L.sortBasedOnPerm(z, l);
						return {
							outputs: [{
								dims: E,
			...
			fe(), we(), Qe(), be(), on = "[a-zA-Z]|\\.\\.\\.", oa = "(" + on + ")+", Qi = "^" + oa + "$", cc = "(" + oa + ",)*" + oa, mc = "^" + cc + "$", fc = class {
				...
				let n = e.map((c) => c.length).map((c, h) => G(`input${h}`, t, c)), i = L.size(a), s = oe("output", t, a.length), l = [...r.symbolToInfo.keys()].filter((c) => !r.rhs.symbolToIndices.has(c)), d = (c) => {
...
`)};
			...
			fe(), we(), be(), bc = (e) => {
				...
				let t = e[0].dims, r = Array.from(e[1].getBigInt64Array(), Number), a = vc(t, r), n = e[0].dataType, i = n === 9 || L.size(t) === 1, s = n === 9 || t.length > 0 && t[t.length - 1] % 4 === 0 ? 4 : 1, l = i || a.length > 0 && a[a.length - 1] % 4 === 0 ? 4 : 1, d = Math.ceil(L.size(a) / l), c = (f) => {
    ...
    ${$}`;
			...
			"use strict";
			fe(), we(), be(), Ja(), Sc = (e) => {
				let t = e[0].dataType, r = L.size(e[0].dims), a = L.size(e[1].dims), n = a % 4 === 0, i = (s) => {
    ...
    ${s.mainStart(Pr)}
      ...
      ${c.setByOffset("global_idx", Ui("x_in"))}
				...
				e.inputs.length < 2 || L.size(e.inputs[1].dims) === 0 ? Hd(e) : e.compute(Sc(e.inputs));
			};
		}), Rc = Z(() => {
			"use strict";
			fe(), we(), Qe(), be(), Ic = (e) => {
				...
				let r = e[0].dims, a = e[1].dims, n = r.length, i = L.normalizeAxis(t.axis, n), s = r.slice(0);
				s.splice(i, 1, ...a);
				let l = r[i], d = e[0].dataType === 9 ? 4 : 1, c = Math.ceil(L.size(s) / d), h = [
					{
						type: 12,
						data: c
					},
  ...
  ${w.mainStart()}
    ...
    input_slice_offsets_data[global_idx] =  base_offset + u32(relative_slice_offset);
				...
				let r = e.inputs, a = r[0].dims, n = r[0].dataType, i = r[1].dims, s = i[i.length - 1], l = L.sizeToDimension(i, i.length - 1), d = L.sizeFromDimension(a, t.batchDims + s), c = L.sizeToDimension(a, t.batchDims), h = L.sizeFromDimension(a, t.batchDims), f = l / c, w = new Array(s), v = d;
				...
				let C = i.slice(0, -1).concat(a.slice(k)), I = L.size(C), E = [
					{
						type: 12,
						data: I
					},
          ...
          output[global_idx] = data[u32(slice_offset) + global_idx % uniforms.slice_size];
			...
			fe(), we(), Qe(), be(), Nc = (e, t) => {
				...
				let r = L.normalizeAxis(t.quantizeAxis, e[0].dims.length), a = t.blockSize, n = e[0], i = e[2], s = e.length === 4 ? e[3] : void 0;
				...
				let r = e[0].dims, a = e[1].dims, n = r.length, i = L.normalizeAxis(t.gatherAxis, n), s = L.normalizeAxis(t.quantizeAxis, n), l = r.slice(0);
				l.splice(i, 1, ...a);
				let d = L.size(l), c = e[2].dataType, h = e[0].dataType === 22, f = [
					{
						type: 12,
						data: d
					},
        ...
        ${E.setByOffset("global_idx", "dequantized_data")};
			...
			fe(), we(), Qe(), be(), qc = (e) => {
				...
				let r = e[0].dims, a = e[0].dataType, n = r.length, i = e[1].dims, s = e[1].dataType, l = L.normalizeAxis(t.axis, n), d = r[l], c = i.slice(0), h = L.size(c), f = G("input", a, n), w = G("indicesInput", s, i.length), v = oe("output", a, c.length), $ = [
					{
						type: 12,
						data: h
					},
    ...
    ${h.guardAgainstOutOfBoundsWorkgroupSizes("uniforms.output_size")}
      ...
      ${nm(s, l, t)}
			...
			fe(), we(), Qe(), Ga(), Ya(), be(), Wt(), dt = (e, t) => e.length > t && e[t].dims.length > 0 ? e[t] : void 0, lm = (e, t) => {
				...
				if (d && c && L.size(d.dims) && L.size(c.dims)) {
					...
					$ = d.dims[2], k = d.dims[2];
				} else if (d && L.size(d.dims) || c && L.size(c.dims)) throw new Error("Input \"past_key\" and \"past_value\" shall be both present or both absent");
				let I;
				if (a && L.size(a.dims) > 0) {
				...
				let N = !1;
				if (s && L.size(s.dims) > 0) throw new Error("Key padding mask is not supported");
				if (l && L.size(l.dims) > 0) {
...
`)}
				...
				let r = e[0].dims, a = L.size(r), n = e[0].dataType, i = L.normalizeAxis(t.axis, r.length), s = new Array(t.numOutputs), l = G("input", n, r.length), d = new Array(t.numOutputs), c = [], h = [], f = 0, w = [{
					type: 12,
					data: a
				}];
  ...
  ${$.mainStart()}
    ...
    writeBufferData(output_number, indices, global_idx);
			...
			fe(), we(), Qe(), be(), wm = (e, t) => {
				...
				if (!L.areEqual(a.dims, []) && !L.areEqual(a.dims, [1]) && a.dims.length !== 2) throw new Error(`Input 'position_ids' is expected to have 0, 1, or 2 dimensions, got ${a.dims.length}`);
				...
				if (!L.areEqual(n.dims, i.dims)) throw new Error("Inputs 'cos_cache' and 'sin_cache' are expected to have the same shape");
				if (l > 0 && s === 0) throw new Error("num_heads must be provided if rotary_embedding_dim is specified");
				let d = r.dims[0], c = r.dims[r.dims.length - 2], h = n.dims[0], f = L.sizeFromDimension(r.dims, 1) / c, w = l === 0 ? n.dims[1] * 2 : f / s;
				...
				let { interleaved: r, numHeads: a, rotaryEmbeddingDim: n, scale: i } = t, s = e[0].dims[0], l = L.sizeFromDimension(e[0].dims, 1), d = e[0].dims[e[0].dims.length - 2], c = l / d, h = e[2].dims[1], f = n === 0 ? h * 2 : c / a, w = new Array(s, d, c / f, f - h), v = L.computeStrides(w), $ = [
					{
						type: 1,
						data: i
					},
            ...
            ${B.setByOffset("k", I.getByOffset("k"))}
				...
				return {
					...
					getRunData: () => ({
						...
						dispatchGroup: { x: Math.ceil(L.size(w) / Pr) },
						programUniforms: $
					})
				};
			}, bm = (e, t) => {
  ...
  ${E.mainStart(w)}
    ...
    if (local_idx == 0) {
      ...
      output[workgroup_index] = vec2f(channel_scale, channel_shift);
				...
				let a = t[0].dims, n = a, i = 2, s = a[0], l = a[1], d = L.sizeFromDimension(a, i), c = Je(d), h = L.size(n) / c, f = os(e, t[0], t[1], t[2], s, d, l, r.epsilon), w = [
					s,
					l,
					d / c
				], v = [s, l], $ = ["type", "none"], k = (C) => {
  ...
  ${C.guardAgainstOutOfBoundsWorkgroupSizes("uniforms.output_size")}
      ...
      ${A.setByOffset("global_idx", "value")};
				...
				let a = t[0].dims, n = a, i = a[0], s = a[a.length - 1], l = L.sizeFromDimension(a, 1) / s, d = Je(s), c = L.size(n) / d, h = [{
					type: 12,
					data: l
				}, {
					type: 12,
  ...
  ${I.mainStart()}
    ...
    output[global_idx] = fma(input[global_idx], ${z(0)}, ${z(1)});
			...
			fe(), we(), be(), Cm = (e) => {
				...
				let a = t.simplified, n = e[0].dims, i = e[1], s = !a && e[2], l = n, d = L.normalizeAxis(t.axis, n.length), c = L.sizeToDimension(n, d), h = L.sizeFromDimension(n, d), f = L.size(i.dims), w = s ? L.size(s.dims) : 0;
  ...
  ${B.mainStart()}
    ...
    ${E ? "inv_std_output[global_idx] = inv_std_dev" : ""};
			...
			we(), tn(), an(), Bm = (e) => {
				...
				if (r < 8 && a < 8) e.compute(Li(e.inputs, { activation: "" }, t));
				else {
					let n = t[t.length - 2], i = L.size(e.inputs[0].dims.slice(0, -2)), s = L.size(e.inputs[1].dims.slice(0, -2));
					if (i !== 1 && n === 1 && s === 1) {
			...
			fe(), we(), Qe(), be(), Nm = (e, t) => {
				...
				if (!L.areEqual(s.dims, [
					t.n,
					n,
					i
				])) throw new Error("The second inputs must be 3D tensor with shape N X nBlocksPerCol X blobSize");
				let l = e[2].dims;
				if (L.size(l) !== t.n * n) throw new Error("scales input size error.");
				if (e.length === 4) {
					let d = e[3].dims, c = t.n * (t.bits === 8 ? n : Math.floor((n * t.bits + 7) / 8));
					if (L.size(d) !== c) throw new Error("zeroPoints input size error.");
				}
			}, Pm = (e, t) => {
				let r = e[0].dims, a = r.length, n = r[a - 2], i = t.k, s = t.n, l = r.slice(0, a - 2), d = L.size(l), c = e[1].dims[2] / 4, h = e[0].dataType, f = Je(t.k), w = Je(c), v = Je(s), $ = l.concat([n, s]), k = n > 1 && s / v % 2 === 0 ? 2 : 1, C = L.size($) / v / k, I = 64, E = [], A = [
					d,
					n,
					i / f
				], z = L.convertShape(e[1].dims).slice();
				z.splice(-1, 1, c / w), E.push(...de(A)), E.push(...de(z)), E.push(...de(e[2].dims)), e.length === 4 && E.push(...de(L.convertShape(e[3].dims)));
				let B = [
					d,
					n,
					s / v
        ...
        ${P.mainStart([
          ...
          if (local_id.x < ${k}) {
            ...
            ${j.setByIndices(`${j.type.indices}(batch, row, col + local_id.x)`, "output_value")};
				...
				let r = e[0].dims, a = r.length, n = r[a - 2], i = t.k, s = t.n, l = r.slice(0, a - 2), d = L.size(l), c = e[1].dims[2] / 4, h = e[0].dataType, f = Je(t.k), w = Je(c), v = l.concat([n, s]), $ = 128, k = s % 8 === 0 ? 8 : s % 4 === 0 ? 4 : 1, C = $ / k, I = Math.floor(32 / t.bits), E = C * w * I, A = E / f, z = E / t.blockSize, B = L.size(v) / k, N = [], P = [
					d,
					n,
					i / f
				], T = L.convertShape(e[1].dims).slice();
				T.splice(-1, 1, c / w), N.push(...de(P)), N.push(...de(T)), N.push(...de(e[2].dims)), e.length === 4 && N.push(...de(L.convertShape(e[3].dims)));
				let Y = [
					d,
					n,
					s
      ...
      `;
          ...
          `;
				...
				let r = L.padShape(e[0].dims.slice(), t.pads), a = e[0].dims, n = [{
					type: 12,
					data: L.size(r)
				}, {
					type: 6,
            ...
            output[global_idx] = value;
				...
				return {
					...
					getRunData: () => ({
						...
						dispatchGroup: { x: Math.ceil(L.size(r) / 64) },
						programUniforms: n
					}),
					getShaderSource: l
				};
			...
			ht(), fe(), we(), be(), la = (e) => {
				...
				let r = t.format === "NHWC", a = L.size(e), n = L.size(t.kernelShape), i = [{
					type: 12,
					data: a
				}, {
					type: 12,
				...
				if (t.kernelShape.length <= 2) {
					...
					let l = L.computeStrides(t.kernelShape);
					return i.push({
						type: 12,
						data: l
					}, {
              ...
              output[global_idx] = value;
				...
				return h.push(...de(t.dims, i)), {
					...
					getRunData: () => ({
						...
						dispatchGroup: { x: Math.ceil(L.size(i) / 64) },
						programUniforms: h
					}),
    ...
    `, l = "", d = G("x", t.dataType, t.dims.length), c = ["rank"], [h, f, w, v, $] = ls(i, n);
			...
			fe(), we(), Qe(), be(), pf = (e, t) => {
				...
				let r = L.normalizeAxis(t.axis, e[0].dims.length), a = e[0].dataType, n = a === 3, i = e[0].dims, s = e[1].dataType, l = L.size(i), d = a === 3 || a === 2, c = d ? [Math.ceil(L.size(e[0].dims) / 4)] : e[0].dims, h = e[1].dims, f = e.length > 2 ? e[2] : void 0, w = f ? d ? [Math.ceil(L.size(f.dims) / 4)] : f.dims : void 0, v = h.length === 0 || h.length === 1 && h[0] === 1, $ = v === !1 && h.length === 1, k = Je(l), C = v && (!d || k === 4), I = C ? k : 1, E = C && !d ? k : 1, A = G("input", d ? 12 : a, c.length, E), z = G("scale", s, h.length), B = f ? G("zero_point", d ? 12 : a, w.length) : void 0, N = oe("output", s, i.length, I), P = [A, z];
				B && P.push(B);
				let T = [c, h];
				f && T.push(w);
				let Y = [
        ...
        output[global_idx] = uniforms.start + ${f}(global_idx) * uniforms.delta;
			...
			fe(), we(), Qe(), be(), bf = (e, t, r, a) => {
              ...
              ${n}bitcast<${a}>(oldValue) + (${r})${i}`;
				...
				let r = e[0].dims, a = e[1].dims, n = r, i = 1, s = Math.ceil(L.sizeToDimension(a, a.length - 1) / i), l = a[a.length - 1], d = L.sizeFromDimension(r, l), c = [
					{
						type: 12,
						data: s
					},
    ...
    depth = max(0, min(depth, ${r[s]} - 1));
      ...
      return (x111 * dx2 * dy2 * dz2 + x112 * dx2 * dy2 * dz1 + x121 * dx2 * dy1 *dz2 + x122 * dx2 * dy1 * dz1 +
              x211 * dx1 * dy2 * dz2 + x212 * dx1 * dy2 * dz1 + x221 * dx1 * dy1 *dz2 + x222 * dx1 * dy1 * dz1);
				...
				let h = oe("output", e.dataType, d.length), f = G("input", e.dataType, s.length), w = L.size(d), v = s.length === d.length && s.every((E, A) => E === d[A]), $ = t.coordinateTransformMode === "tf_crop_and_resize", k = t.extrapolationValue, C = f.type.value, I = (E) => `
      ${v ? "" : `
...
`}
			...
			fe(), we(), be(), Gf = (e) => {
				...
				let n = t.simplified, i = e[0].dims, s = L.size(i), l = i, d = s, c = i.slice(-1)[0], h = a ? i.slice(0, -1).concat(1) : [], f = !n && e.length > 3, w = e.length > 4, v = a && r > 1, $ = a && r > 2, k = r > 3, C = 64, I = Je(c), E = [
					{
						type: 12,
						data: d
					},
      ...
      ${B.mainStart([
        ...
        for (var i: u32 = 0; i < stride; i++) {
          ...
          return input_indices;
				...
				let r = e[0].dims, a = L.size(r), n = t.axes.length > 0 ? L.normalizeAxes(t.axes, r.length) : [...Array(r.length).keys()], i = da(e, 4);
				i.forEach((I) => I !== 0 || (() => {
				...
				let h = {
					dims: c,
					dataType: e[0].dataType
				}, f = oe("output", e[0].dataType, c.length), w = G("input", e[0].dataType, e[0].dims.length), v = L.size(c), $ = [
					{
						name: "outputSize",
						type: "u32"
					},
        ...
        ${I.mainStart()}
          ...
          ${f.setByOffset("global_idx", w.getByIndices("input_indices"))}
			...
			fe(), we(), Qe(), Wt(), be(), th = (e) => {
				...
				let r = e.inputs[0], a = r.dims, n = L.size(a), i = a.length, s = L.normalizeAxis(t.axis, i), l = s < a.length - 1, d, c = [];
        ...
        for (var col = lindex; col < cols; col += wg) {
          ...
          setValue(row, col, row_stride, value);
			...
			fe(), we(), be(), ws = (e) => Array.from(e.getBigInt64Array(), Number), sh = (e) => {
				...
				let r = e[0].dims, a = t ?? ws(e[1]), n = oh(r, a), i = L.size(n), s = e[0].dataType, l = G("input", s, r.length), d = oe("output", s, n.length), c = (h) => `
      ...
      ${d.setByOffset("global_idx", l.getByIndices("input_indices"))}
        ...
        ${c}
				...
				let t = e[1].dims, r = e[2].dims, a = e[0].dims, n = e[1].dataType, i = !(L.areEqual(t, r) && L.areEqual(r, a)), s = t, l = L.size(t);
				if (i) {
			...
			"use strict";
			Yl(), Ya(), ud(), cd(), tp(), fp(), vp(), Vp(), tc(), ic(), pc(), wc(), kc(), Tc(), Rc(), Mc(), Lc(), Hc(), Yc(), um(), Sm(), zm(), Om(), Mm(), qm(), rs(), Qm(), df(), hf(), wf(), kf(), Xa(), Wf(), is(), jf(), eh(), ih(), ns(), dh(), Wt(), Ja(), fh(), hh = /* @__PURE__ */ new Map([
				["Abs", [fd]],
				["Acos", [hd]],
				["Acosh", [gd]],
				["Add", [np]],
...
${i}`, l = r.createShaderModule({
			...
			fe(), Mt(), we(), Gu(), ln = class f0 {
				...
				reshape(t) {
					if (L.size(t) !== L.size(this.dims)) throw new Error("Invalid new shape");
			...
			"use strict";
			ht(), vu(), Tu(), fe(), rr(), Fa(), _i(), Th = (e, t) => {
				We()._OrtInit(e, t) !== 0 && Fe("Can't initialize onnxruntime.");
			}, $s = async (e) => {
				...
				try {
					let a = t.PTR_SIZE, n = t.stackAlloc(2 * a);
					t._OrtGetInputOutputCount(e, n, n + a) !== 0 && Fe("Can't get session input/output count.");
					let i = a === 4 ? "i32" : "i64";
				...
				try {
					...
					r._OrtGetInputOutputMetadata(e, t, s, s + i) !== 0 && Fe("Can't get session input/output metadata.");
				...
				try {
					...
					i = await n._OrtCreateSession(r, a, s), n.webgpuOnCreateSession?.(i), i === 0 && Fe("Can't create a session."), n.jsepOnCreateSession?.(), n.currentContext && (n.webnnRegisterMLContext(i, n.currentContext), n.currentContext = void 0, n.shouldTransferToMLTensor = !0);
					let [f, w] = Ih(i), v = !!t?.enableGraphCapture, $ = [], k = [], C = [], I = [], E = [];
					for (let z = 0; z < f; z++) {
						let [B, N, P] = ks(i, z);
						B === 0 && Fe("Can't get an input name."), c.push(B);
					...
					for (let z = 0; z < w; z++) {
						let [B, N, P] = ks(i, z + f);
						B === 0 && Fe("Can't get an output name."), h.push(B);
						let T = n.UTF8ToString(B);
						k.push(T), I.push(N === 0 ? {
							name: T,
							isTensor: !1
					...
					let A = null;
					return E.some((z) => z === "gpu-buffer" || z === "ml-tensor" || z === "ml-tensor-cpu-output") && (l = n._OrtCreateBinding(i), l === 0 && Fe("Can't create IO binding."), A = {
						handle: l,
					...
					throw c.forEach((w) => n._OrtFree(w)), h.forEach((w) => n._OrtFree(w)), l !== 0 && n._OrtReleaseBinding(l) !== 0 && Fe("Can't release IO binding."), i !== 0 && n._OrtReleaseSession(i) !== 0 && Fe("Can't release session."), f;
				} finally {
					n._free(r), s !== 0 && n._OrtReleaseSessionOptions(s) !== 0 && Fe("Can't release session options."), d.forEach((f) => n._free(f)), n.unmountExternalData?.();
				}
			}, Es = (e) => {
				let t = We(), r = ir.get(e);
				if (!r) throw new Error(`cannot release session. invalid session id: ${e}`);
				let [a, n, i, s, l] = r;
				s && (l && t._OrtClearBoundOutputs(s.handle) !== 0 && Fe("Can't clear bound outputs."), t._OrtReleaseBinding(s.handle) !== 0 && Fe("Can't release IO binding.")), t.jsepOnReleaseSession?.(e), t.webnnOnReleaseSession?.(e), t.webgpuOnReleaseSession?.(e), n.forEach((d) => t._OrtFree(d)), i.forEach((d) => t._OrtFree(d)), t._OrtReleaseSession(a) !== 0 && Fe("Can't release session."), ir.delete(e);
				...
				try {
					...
					I === 0 && Fe(`Can't create tensor for input/output. session=${a}, index=${i}.`), t.push(I);
				} finally {
					l.stackRestore(k);
				}
				...
				try {
					...
					if (w && !$) {
						...
						for (let he = 0; he < k; he++) {
							let ge = t[he];
							await s._OrtBindInput(j, h[ge], A[he]) !== 0 && Fe(`Can't bind input[${he}] for session=${e}.`);
						}
						for (let he = 0; he < C; he++) {
							let ge = a[he];
							n[he]?.[3] ? (N.push(z[he]), s._OrtBindOutput(j, f[ge], z[he], 0) !== 0 && Fe(`Can't bind pre-allocated output[${he}] for session=${e}.`)) : s._OrtBindOutput(j, f[ge], 0, pe[ge]) !== 0 && Fe(`Can't bind output[${he}] to ${ne[he]} for session=${e}.`);
						}
					...
					let ie;
					w ? ie = await s._OrtRunWithBinding(c, w.handle, C, X, I) : ie = await s._OrtRun(c, Y, T, k, ue, C, X, I), ie !== 0 && Fe("failed to call OrtRun().");
					let K = [], U = [];
					er("wasm ProcessOutputTensor");
					for (let j = 0; j < C; j++) {
						let ne = Number(s.getValue(X + j * l, "*"));
						if (ne === z[j] || N.includes(z[j])) {
							K.push(n[j]), ne !== z[j] && s._OrtReleaseTensor(ne) !== 0 && Fe("Can't release tensor.");
							continue;
						}
						let pe = s.stackSave(), he = s.stackAlloc(4 * l), ge = !1, V, se = 0;
						try {
							s._OrtGetTensorData(ne, he, he + l, he + 2 * l, he + 3 * l) !== 0 && Fe(`Can't access output tensor data on index ${j}.`);
							...
							s._OrtFree(ee) !== 0 && Fe("Can't free memory for tensor dims.");
							...
							if (V === "string") {
								...
								ge = !0, K.push([
									...
									ot,
									{
										...
										dispose: () => {
											s._OrtReleaseTensor(ne) !== 0 && Fe("Can't release tensor.");
										}
									},
									"gpu-buffer"
								]);
					...
					w && !v && (s._OrtClearBoundOutputs(w.handle) !== 0 && Fe("Can't clear bound outputs."), ir.set(e, [
						c,
						h,
						f,
						w,
				...
				n === 0 && Fe("Can't get an profile file name."), t._OrtFree(n);
			}, Cs = (e) => {
				let t = [];
				for (let r of e) {
					let a = r[2];
			...
			"use strict";
			ht(), As(), rr(), Na(), sr = () => !!He.wasm.proxy && typeof document < "u", Fr = !1, pa = !1, ca = !1, cn = /* @__PURE__ */ new Map(), Er = (e, t) => {
				let r = cn.get(e);
				r ? r.push(t) : cn.set(e, [t]);
			}, Tr = () => {
				if (Fr || !pa || ca || !wt) throw new Error("worker not ready");
			}, zh = (e) => {
				switch (e.data.type) {
					case "init-wasm":
						Fr = !1, e.data.err ? (ca = !0, Rs[1](e.data.err)) : (pa = !0, Rs[0]()), pn && (URL.revokeObjectURL(pn), pn = void 0);
						break;
					case "init-ep":
					case "copy-from":
					case "create":
				...
				if (!pa) {
					if (Fr) throw new Error("multiple calls to 'initWasm()' detected.");
					if (ca) throw new Error("previous call to 'initWasm()' failed.");
					if (Fr = !0, sr()) return new Promise((e, t) => {
	...
	const Ps = {
		...
		constructor({ voiceId: t, progress: r, logger: a, wasmPaths: n }) {
			ma(this, "ready", !1), ma(this, "voiceId", "en_US-hfc_female-medium"), ma(this, "waitReady", !1), Ir(this, mn), Ir(this, Vt), Ir(this, At), Ir(this, fn), Ir(this, Lr), Ir(this, zr, Ps), Ir(this, hn);
			var i;
			return Qt._instance ? (a?.("Reusing session for TTS!"), Qt._instance.voiceId = t ?? Qt._instance.voiceId, or(Qt._instance, Lr, r ?? et(Qt._instance, Lr)), Qt._instance) : (a?.("New session"), or(this, hn, a), this.voiceId = t, or(this, Lr, r), this.waitReady = this.init(), or(this, zr, n ?? Ps), (i = et(this, hn)) == null || i.call(this, `Loaded WASMPaths at: ${JSON.stringify(et(this, zr))}`), Qt._instance = this, this);
		}
		...
		async init() {
			...
			const n = await Kh(`${gn}/${r}`, et(this, Lr));
	...
	mn = /* @__PURE__ */ new WeakMap(), Vt = /* @__PURE__ */ new WeakMap(), At = /* @__PURE__ */ new WeakMap(), fn = /* @__PURE__ */ new WeakMap(), Lr = /* @__PURE__ */ new WeakMap(), zr = /* @__PURE__ */ new WeakMap(), hn = /* @__PURE__ */ new WeakMap(), ma(Us, "WASM_LOCATIONS", Ps), ma(Us, "_instance", null);
	let I0 = Us;
	...
	const C0 = "RIFF", A0 = "WAVE", R0 = "fmt ", O0 = "data";
	function Fs(e, t) {
		return String.fromCharCode(...e.subarray(t, t + 4));
	}
	function B0(e) {
		const t = new Uint8Array(e);
		if (t.byteLength < 44 || Fs(t, 0) !== C0 || Fs(t, 8) !== A0) return null;
	...
	function D0(e, t = {}) {
		const r = B0(e), a = t.leadingSilenceMs ?? 140, n = t.trailingSilenceMs ?? 80;
	...
	async function Qh(e) {
		try {
			const i = await caches.open("piper-voices-v1"), [s, l] = await Promise.all([i.match(e.modelUrl), i.match(e.configUrl)]);
			if (s && l && (await s.blob()).size > 0) return !0;
		} catch {}
	...
	function P0() {
		Xh || (Xh = !0, self.fetch = (async (e, t) => {
			...
			if (r.includes("en_US-lessac-medium.onnx")) {
				try {
					const n = await (await caches.open("piper-voices-v1")).match(r);
					if (n) return n.clone();
				} catch {}
				const a = qs;
				if (a) {
	...
	async function Ws(e) {
		try {
			...
			if (t?.name === "NotFoundError" || t?.name === "EntryNotFound" || t?.message?.includes?.("NotFoundError") || t?.message?.includes?.("could not be found")) {
				console.info(`[localTts.worker] Mô hình '${e}' chưa tồn tại trong OPFS hoặc đã được dọn dẹp trước đó.`);
				return;
			}
			console.warn(`[localTts.worker] Lỗi không nghiêm trọng khi dọn OPFS '${e}':`, t);
		}
	}
	async function U0(e, t) {
		return await N0(e, t), P0(), fa && Ls === t.version || (await Ws(yn), Ls = t.version, qs = t, wn(e, { phase: "load" }), fa = await I0.create({
			voiceId: yn,
			progress: (r) => wn(e, {
				phase: "download",
				loaded: r.loaded,
		...
		}), await Ws(yn), wn(e, { phase: "ready" })), fa;
	}
	self.onmessage = async (e) => {
		const { id: t, type: r, text: a, config: n } = e.data;
		try {
			if (r === "remove") {
				await Ws(yn), fa = null, Ls = null, qs = null, self.postMessage({
					id: t,
					type: "result"
				});
				return;
			}
			const i = await U0(t, n);
			if (r === "prepare") {
				self.postMessage({
					id: t,
					type: "result"
			...
			const l = await M0(await i.predict(a || ""));
			self.postMessage({
				id: t,
				type: "result",
				blob: l
			...
			if (i?.name === "NotFoundError" || i?.name === "EntryNotFound" || i?.message?.includes?.("NotFoundError") || i?.message?.includes?.("could not be found")) {
				console.info("[localTts.worker] Không tìm thấy mô hình TTS ngoại tuyến (NotFoundError), báo hiệu fallback sang Web Speech API."), self.postMessage({
					id: t,
					type: "error",
					message: "Mô hình Local TTS chưa được cài đặt (NotFoundError). Đang chuyển sang giọng đọc trình duyệt."
				});
				return;
			}
			self.postMessage({
