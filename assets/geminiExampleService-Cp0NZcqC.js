...
]`,m=async(e=`gemini-3.5-flash`)=>{let t=`https://generativelanguage.googleapis.com/v1beta/models/${e}:generateContent?key=${encodeURIComponent(l)}`,n=await fetch(t,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({contents:[{parts:[{text:p}]}],generationConfig:{responseMimeType:`application/json`,temperature:.4}})});return n.status===404&&e===`gemini-3.5-flash`?m(`gemini-2.5-flash`):n},h=await m();if(!h.ok){let e=await h.json().catch(()=>({}));throw Error(e?.error?.message||`Gemini API error: ${h.status}`)}let g=(await h.json())?.candidates?.[0]?.content?.parts?.[0]?.text;if(!g)throw Error(`Không nhận được phản hồi từ AI Gemini.`);let _=JSON.parse(g);if(!Array.isArray(_)||_.length===0)throw Error(`Định dạng dữ liệu trả về từ AI không hợp lệ.`);return _};export{t as n,n as t};